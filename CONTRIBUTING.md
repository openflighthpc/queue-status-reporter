# Contributing to queue-status-reporter

:+1::tada: Thanks for taking the time to contribute! :tada::+1:

You want to contribute to queue-status-reporter? Welcome! Please read this
...
You can help queue-status-reporter by helping others who use it and need support.

## Analyse Issues

Analysing issue reports can be a lot of effort. Any help is welcome!
Go to [the GitHub issue tracker](https://github.com/openflighthpc/queue-status-reporter/issues?state=open)
...
If you find a bug - behaviour of queue-status-reporter code or documentation
...
[GitHub issue tracker for queue-status-reporter](https://github.com/openflighthpc/queue-status-reporter/issues/new)
to report the issue.
...
We want to improve the quality of queue-status-reporter and good bug reports are
...
queue-status-reporter is Open Source and comes without warranty.
...
queue-status-reporter in order to fix bugs or to implement new features.
...
2. **Not all proposed contributions can be accepted**. Some features
   ...
   the overall direction of queue-status-reporter and really improve it. The more
...
applied to queue-status-reporter itself.

In particular you need to agree to the Contributor License Agreement,
which can be [found
here](https://www.clahub.com/agreements/openflighthpc/queue-status-reporter). This
...
8. Wait for our code review and approval, possibly enhancing your
   change on request.
    - Note that the queue-status-reporter developers also have their regular
